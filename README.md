...
`EventsService` использует Redis для кеширования часто запрашиваемых данных. Доступ к кешу вынесен в абстракцию `ICacheService` в слое Application, а реализация на `StackExchange.Redis` находится в Infrastructure. Для чтения используется паттерн Cache-Aside: сначала сервис проверяет кеш, при промахе получает данные из PostgreSQL и сохраняет результат в Redis с заданным TTL.

Параметры Redis и время жизни кеша находятся в `EventsService.Presentation/appsettings.json`:
...
Для отдельного мероприятия используется ключ:

```text
event:{id}
```

Для списка популярных мероприятий:

```text
events:top10
```

### Кеш отдельного мероприятия

Для `GET /events/{id}` применяется Cache-Aside с инвалидацией при записи.

При попадании в кеш данные возвращаются без обращения к PostgreSQL. При промахе мероприятие загружается из базы и сохраняется в Redis на время, заданное `EventTtlMinutes`.

Для отдельного мероприятия выбрана стратегия **инвалидации при записи**, а не обновления кеша вместе с базой. После успешного создания, изменения или удаления мероприятия соответствующий ключ `event:{id}` удаляется. Следующий запрос получает актуальные данные из PostgreSQL и заново прогревает кеш.

Такой подход выбран потому, что PostgreSQL остаётся единственным источником истины, а в коде не требуется отдельно поддерживать две версии одного объекта — сохранённую в БД и записываемую в Redis.

Количество доступных мест может изменяться не только через HTTP-операции с мероприятием, но и в `BookingLifecycleHandler` при обработке Kafka-сообщений. Поэтому после успешного резервирования или освобождения мест ключ соответствующего мероприятия также инвалидируется.

Изменения всегда сначала сохраняются в PostgreSQL и только после этого инвалидируется кеш. Если операция с базой завершается ошибкой, кеш не изменяется. Если процесс завершится после сохранения данных в PostgreSQL, но до удаления ключа Redis, старое значение может сохраняться до истечения TTL; PostgreSQL при этом остаётся актуальным источником данных.

### Кеш топ-10 мероприятий

`GET /events/top` возвращает 10 мероприятий с наибольшим процентом проданных мест:

```text
(TotalSeats - AvailableSeats) / TotalSeats
```

Результат кешируется по ключу `events:top10`. Для этого агрегата используется только обновление по TTL, без явной инвалидации при каждом изменении мероприятия или бронировании.

Небольшое временное устаревание рейтинга допустимо, а инвалидация `events:top10` при каждом резервировании или освобождении места приводила бы к частому сбросу кеша и дополнительным обращениям к Redis и PostgreSQL. Поэтому список пересчитывается после истечения `TopEventsTtlMinutes`.

### Отказ Redis

Redis рассматривается как вспомогательный кеш, а не как источник данных. Если Redis временно недоступен, ошибка логируется и не передаётся клиенту. Чтение продолжает выполняться из PostgreSQL, а ошибки записи или удаления кеша не отменяют успешно выполненные операции с основной базой данных.
